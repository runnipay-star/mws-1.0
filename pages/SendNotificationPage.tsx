...
const SendNotificationPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccess(`Notifica inviata con successo a ${selectedUserIds.size} utente(i).`);
            setTitle('');
            setMessage('');
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 max-w-4xl mx-auto">
            <div className="flex items-center space-x-3 mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">
                ...
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Invia Notifica</h2>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label className="block text-lg font-semibold text-slate-800 dark:text-white mb-3">Destinatari</label>
                    {isFetchingUsers ? (
                         <div className="flex items-center justify-center p-4">
                            <Loader2 className="w-6 h-6 text-primary-500 animate-spin" />
                            <span className="ml-2 text-slate-500">Caricamento utenti...</span>
                        </div>
                    ) : (
                            ...
                            <div className="space-y-3">
                                {adminUser && (
                                    <label className="flex items-center space-x-3 p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm font-medium text-slate-700 dark:text-gray-300">Me Stesso (Admin)</span>
                                    </label>
                                )}
                                {clients.length > 0 && (
                                    <div className="pt-3 border-t border-slate-200 dark:border-slate-700">
                                        <label className="flex items-center space-x-3 p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer font-semibold">
                                            <input
                                                ...
                                                className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                                            />
                                            <span className="text-sm text-slate-800 dark:text-gray-200">Tutti i Clienti</span>
                                        </label>
