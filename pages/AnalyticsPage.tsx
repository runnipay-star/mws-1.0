...
const AdSpendChart: React.FC<{ data: Record<string, number> }> = ({ data }) => {
    // FIX: Operator '+' cannot be applied to types 'unknown' and 'number'. Explicitly type the accumulator.
    const totalSpend = Object.values(data).reduce((sum: number, amount) => sum + Number(amount), 0);
...
const LeadStatusChart: React.FC<{ data: Record<string, number> }> = ({ data }) => {
     ...
     // FIX: Operator '+' cannot be applied to types 'unknown' and 'number'. Explicitly type the accumulator.
     const totalLeads = Object.values(data).reduce((sum: number, count) => sum + Number(count), 0);
...
const AnalyticsPage: React.FC = () => {
    ...
    const handleDateInputChange = (type: 'start' | 'end', value: string) => {
        ...
        if (type === 'start') {
            ...
            const endDate = value ? new Date(`${value}T23:59:59`) : null;
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                        <div className="grid grid-cols-2 gap-2 sm:col-span-2">
                            <div>
                                ...
                                <input type="date" id="start-date" value={toInputDateString(dateRange.start)} onChange={e => handleDateInputChange('start', e.target.value)} className={inputClasses}/>
                            </div>
                             <div>
                                <label htmlFor="end-date" className="block text-sm font-medium text-slate-700 dark:text-gray-300">Al</label>
                                <input type="date" id="end-date" value={toInputDateString(dateRange.end)} onChange={e => handleDateInputChange('end', e.target.value)} className={inputClasses}/>
                            </div>
                        </div>
