...
const AdSpendChart: React.FC<{ data: Record<string, number> }> = ({ data }) => {
    // FIX: Explicitly set generic type for reduce to correctly infer return type as number.
    const totalSpend = Object.values(data).reduce<number>((sum, amount) => sum + amount, 0);
...
const LeadStatusChart: React.FC<{ data: Record<string, number> }> = ({ data }) => {
     ...
     // FIX: Explicitly set generic type for reduce to correctly infer return type as number.
     const totalLeads = Object.values(data).reduce<number>((sum, count) => sum + count, 0);
