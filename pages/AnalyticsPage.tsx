...
const AdSpendChart: React.FC<{ data: Record<string, number> }> = ({ data }) => {
    // FIX: Add explicit types to reduce function to resolve TS inference error.
    const totalSpend = Object.values(data).reduce((sum: number, amount: number) => sum + amount, 0);
...
const LeadStatusChart: React.FC<{ data: Record<string, number> }> = ({ data }) => {
     ...
     // FIX: Add explicit types to reduce function to resolve TS inference error.
     const totalLeads = Object.values(data).reduce((sum: number, count: number) => sum + count, 0);
