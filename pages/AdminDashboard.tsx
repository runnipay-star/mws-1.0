...
const AdminDashboard: React.FC = () => {
    ...
    const [tagliandoModalState, setTagliandoModalState] = useState<{
        isOpen: boolean;
        clientId: string | null;
        leadId: string | null;
        updates: Partial<Lead> | null;
    }>({ isOpen: false, clientId: null, leadId: null, updates: null });
    ...
    const completeLeadUpdateWithNote = async (clientId: string, leadId: string, updates: Partial<Lead>, noteContent: string) => {
        try {
            const client = clients.find(c => c.id === clientId);
            const lead = client?.leads.find(l => l.id === leadId);
            if (!lead) throw new Error("Lead non trovato per la conversione in storico.");
    
            const finalUpdates = {
                ...updates,
                data: { ...lead.data, _is_historical: 'true' }
            };
            
            await ApiService.updateLead(clientId, leadId, finalUpdates);
            await ApiService.addNoteToLead(clientId, leadId, noteContent);
            await fetchData();
        } catch (error) {
            console.error("Fallimento nell'aggiornare il lead con nota e flag storico:", error);
            alert('Si è verificato un errore durante il salvataggio.');
            await fetchData();
        } finally {
            setTagliandoModalState({ isOpen: false, clientId: null, leadId: null, updates: null });
        }
    };
    ...
    const handleLeadUpdate = async (clientId: string, leadId: string, updates: Partial<Lead>) => {
        ...
        if (isBecomingVinto) {
            ...
            const isTagliandiService = lead.service?.toLowerCase().includes('tagliando');
    
            if (isFacchetti && isTagliandiService) {
                setTagliandoModalState({
                    isOpen: true,
                    clientId: clientId,
                    leadId: leadId,
                    updates: updates
    ...
    const filteredLeads = useMemo(() => {
        ...
        if (searchQuery.trim() !== '') {
            ...
            filtered = filtered.filter(({ lead, client }) => 
                // FIX: Cast val to string to use toLowerCase method.
                Object.values(lead.data).some(val => String(val).toLowerCase().includes(lowercasedQuery)) ||
    ...
    const TagliandoTypeModal: React.FC<{
        state: typeof tagliandoModalState;
        onClose: () => void;
        onSubmit: (selectedType: string) => Promise<void>;
    }> = ({ state, onClose, onSubmit }) => {
        const [selectedType, setSelectedType] = useState('');
        const [isSubmitting, setIsSubmitting] = useState(false);
        const tagliandoTypes = [
            'Tagliando motore',
            'Tagliando cambio',
            'Tagliando motore e cambio',
            'Tagliando differenziale 4x4',
            'Tagliando differenziale 4x4 + Cambio',
            'Cinghia di distribuzione',
            'Cinghia di distribuzione e Tagliando motore'
        ];
        
        useEffect(() => {
            if (state.isOpen) {
                setSelectedType(tagliandoTypes[0]);
        ...
        return (
            <Modal isOpen={state.isOpen} onClose={onClose} title="Dettaglio Intervento">
                <div className="space-y-4">
                    <p className="text-slate-600 dark:text-gray-300">
                        Specifica il tipo di tagliando eseguito. Questa informazione verrà aggiunta come nota e il lead verrà archiviato.
                    </p>
                    <div className="space-y-2">
                        {tagliandoTypes.map(type => (
                            <label key={type} className="flex items-center space-x-3 p-3 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer border border-slate-200 dark:border-slate-600">
                                <input
                                    type="radio"
                                    name="tagliandoType"
                                    value={type}
                    ...
                    <div className="mt-6 flex justify-end space-x-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                        ...
                        <button type="button" onClick={handleSubmit} disabled={!selectedType || isSubmitting} className="px-4 py-2 text-sm font-medium rounded-md bg-primary-600 text-white hover:bg-primary-700 disabled:opacity-50">
                            {isSubmitting ? 'Salvataggio...' : 'Conferma e Archivia'}
                        </button>
                    </div>
                </div>
            </Modal>
                                ...
                                {paginatedLeads.map(({ lead, client }) => {
                                    ...
                                    return (
                                        <div key={lead.id} className={`p-4 ${isSelected ? 'bg-primary-50 dark:bg-slate-700' : 'bg-white dark:bg-slate-900'}`}>
                                            <div className="flex justify-between items-start gap-4">
                                                <div className="flex items-center gap-3 flex-grow min-w-0" onClick={() => handleViewLeadDetails(lead, client)}>
                                                    <input
                                                        type="checkbox"
                                                        checked={isSelected}
                                                        onChange={(e) => handleSelectLead(lead.id, e.target.checked)}
                                                    ...
                                                    />
                                                    <div className="min-w-0">
                                                        <div className={`font-semibold text-slate-900 dark:text-white truncate ${lead.status === 'Nuovo' ? 'border-l-4 border-primary-500 pl-2 -ml-2' : ''}`}>{lead.data.nome || 'N/D'}</div>
                                                        <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{client.name} &middot; {lead.service || 'N/D'}</div>
                                                        {lead.data.telefono && (
    ...
    return (
        <div>
            ...
            <TagliandoTypeModal
                state={tagliandoModalState}
                onClose={() => setTagliandoModalState({ isOpen: false, clientId: null, leadId: null, updates: null })}
                onSubmit={async (selectedType) => {
                    if (tagliandoModalState.clientId && tagliandoModalState.leadId && tagliandoModalState.updates) {
                        const noteContent = `Tipo di intervento: ${selectedType}`;
                        await completeLeadUpdateWithNote(tagliandoModalState.clientId, tagliandoModalState.leadId, tagliandoModalState.updates, noteContent);
                    }
                }}
            />
