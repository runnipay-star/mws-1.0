...
const EditNotificationForm: React.FC<{
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!title.trim() || !message.trim()) {
            setError('Titolo e messaggio non possono essere vuoti.');
            return;
        }

        if (!window.confirm("Confermando, la notifica originale verrà eliminata e una nuova versione aggiornata verrà inviata a tutti i destinatari. Continuare?")) {
            return;
        }
        ...
        try {
            ...
            setError(err.message || 'Impossibile modificare e reinviare la notifica.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label htmlFor="edit-title" className="block text-sm font-medium text-slate-700 dark:text-gray-300">Titolo</label>
            ...
            <div>
                <label htmlFor="edit-message" className="block text-sm font-medium text-slate-700 dark:text-gray-300">Messaggio</label>
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                <button type="button" onClick={onCancel} className="bg-slate-200 dark:bg-slate-600 text-slate-800 dark:text-white px-4 py-2 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500">Annulla</button>
                <button type="submit" disabled={isLoading} className="bg-primary-600 text-white px-4 py-2 rounded-lg shadow hover:bg-primary-700 disabled:opacity-50">
                    {isLoading ? 'Salvataggio...' : 'Salva e Reinvia'}
                </button>
            </div>
        </form>
    );
...
const ManageNotificationsPage: React.FC = () => {
    ...
    const handleDelete = async (notification: Notification) => {
        if (window.confirm("Sei sicuro di voler eliminare questa notifica per tutti i destinatari? L'azione è irreversibile.")) {
            try {
                ...
                alert("Impossibile eliminare la notifica. Riprova.");
            }
        }
    };
        ...
        <>
            <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 max-w-6xl mx-auto">
                <div className="flex items-center space-x-3 mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">
                    ...
                    <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Gestione Notifiche Inviate</h2>
                </div>
                
                {sentNotifications.length === 0 ? (
                    <div className="text-center py-20 text-slate-500 dark:text-gray-400">
                        <BellOff className="w-12 h-12 mx-auto mb-4" />
                        <p className="text-lg">Non hai ancora inviato nessuna notifica personalizzata.</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                            <thead className="bg-slate-50 dark:bg-slate-800">
                                <tr>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Titolo</th>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Messaggio (Anteprima)</th>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data Invio</th>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Modifica Notifica">
                {editingNotification && (
