...
const ClientDashboard: React.FC = () => {
    ...
    const completeLeadUpdateWithNote = async (leadId: string, updates: Partial<Lead>, noteContent: string) => {
        if (!client) return;
        try {
            ...
            await ApiService.addNoteToLead(client.id, leadId, noteContent);
            await fetchClientData();
        } catch (error) {
            console.error("Fallimento nell'aggiornare il lead con nota:", error);
            alert('Si è verificato un errore durante il salvataggio.');
            await fetchClientData();
        } finally {
            setTagliandoModalState({ isOpen: false, leadId: null, updates: null });
        }
    };
    ...
    const handleLeadUpdate = async (leadId: string, updates: Partial<Lead>) => {
        ...
        if (isBecomingVinto) {
            ...
            if (isFacchetti && isTagliandiService) {
                setTagliandoModalState({ isOpen: true, leadId: leadId, updates: updates });
                return;
            }
        }
                        ...
                        <table className="min-w-full">
                            ...
                            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200 dark:divide-slate-800 md:divide-y-0">
                                {client.adSpends?.map(spend => {
                                    ...
                                    // FIX: The property 'end_date' does not exist on type 'AdSpend'. The logic has been updated to only use `spend.date`.
    ...
    return (
        <div>
            ...
            <TagliandoTypeModal
                ...
                onClose={() => setTagliandoModalState({ isOpen: false, leadId: null, updates: null })}
                onSubmit={async (selectedType) => {
                    if (tagliandoModalState.leadId && tagliandoModalState.updates) {
                        const noteContent = `Tipo di intervento: ${selectedType}`;
                        await completeLeadUpdateWithNote(tagliandoModalState.leadId, tagliandoModalState.updates, noteContent);
                    }
                }}
            />
