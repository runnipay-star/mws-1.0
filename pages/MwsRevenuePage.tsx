...
const ClientRevenueCalculator: React.FC<ClientRevenueCalculatorProps> = ({ client, dateRange, onClientUpdate, isReadOnly }) => {
    ...
    const calculations = useMemo(() => {
        ...
        const mwsProfitShare = (clientRevenue * mwsProfitPercentage) / 100;
    ...
    return (
        ...
        <tr className="hidden md:table-row hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm">
                <input type="number" value={fee} onChange={e => handleValueChange('fee', e.target.value)} readOnly={isReadOnly} className={`w-24 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md py-1 px-2 text-sm ${isReadOnly ? 'cursor-not-allowed opacity-70' : ''}`} placeholder="0"/>
            </td>
            <td className="px-6 py-4 whitespace-nowrap text-sm">
                <input type="number" value={percentage} onChange={e => handleValueChange('percentage', e.target.value)} readOnly={isReadOnly} className={`w-20 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md py-1 px-2 text-sm ${isReadOnly ? 'cursor-not-allowed opacity-70' : ''}`} placeholder="0"/>
            </td>
        ...
        <div className="md:hidden p-4 rounded-lg shadow border bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 pt-3 border-t border-slate-200 dark:border-slate-700">
                <div>
                    ...
                    <input type="number" value={fee} onChange={e => handleValueChange('fee', e.target.value)} readOnly={isReadOnly} className={`w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md py-1 px-2 text-sm ${isReadOnly ? 'cursor-not-allowed opacity-70' : ''}`} placeholder="0"/>
                </div>
                 <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-gray-400 mb-1">% su Profitto</label>
                    <input type="number" value={percentage} onChange={e => handleValueChange('percentage', e.target.value)} readOnly={isReadOnly} className={`w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md py-1 px-2 text-sm ${isReadOnly ? 'cursor-not-allowed opacity-70' : ''}`} placeholder="0"/>
                </div>
            </div>
             <div className="flex justify-between items-center pt-3 border-t border-slate-200 dark:border-slate-700">
                ...
                {!isReadOnly && <div className="p-2 w-10 h-10 flex items-center justify-center">{renderStatusIndicator()}</div>}
            </div>
        </div>
        </>
    );
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (user?.role === 'admin') {
                ...
                setClients(clientData ? [clientData] : []);
            } else {
                setClients([]);
            }
        } catch (error) {
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-sm hidden md:table">
                        ...
                        <tfoot className="bg-slate-100 dark:bg-slate-950 font-bold border-t-2 border-slate-300 dark:border-slate-700">
                            <tr>
                                <td colSpan={isClientView ? 6 : 7} className="px-6 py-4 text-right text-sm text-slate-800 dark:text-white uppercase">Totale Fatturato MWS</td>
