...
export class ApiService {
    ...
    static async updateSentNotification(originalNotification: Notification, newTitle: string, newMessage: string): Promise<void> {
        ...
        if (fetchError) {
            throw new Error(`Impossibile recuperare le notifiche originali: ${fetchError.message}`);
        }
    
        if (!originalBatch || originalBatch.length === 0) {
            console.warn("Nessuna notifica trovata per il gruppo da aggiornare.");
            return;
        }
        ...
        if (deleteError) {
            throw new Error(`Impossibile eliminare le notifiche originali: ${deleteError.message}`);
        }
        ...
        if (recipientUserIds.length > 0) {
            ...
            if (insertError) {
                throw new Error(`Impossibile reinviare le nuove notifiche: ${insertError.message}`);
            }
        }
    }
