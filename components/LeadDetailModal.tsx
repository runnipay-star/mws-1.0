...
import { Tag, Calendar, Info, DollarSign, Briefcase, MessageCircle, History, Sparkles, Copy, Loader2, Check, Phone, Edit, Trash2, Mail, Save, X } from 'lucide-react';
...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ isOpen, onClose, lead, client, historicalLeads, onAddNote, onUpdateNote, onDeleteNote, onHistoricalLeadAdded, onHistoricalLeadUpdated, onHistoricalLeadDeleted }) => {
                        ...
                        {allSortedEntries.map(([key, value]) => (
                            <div key={key} className="py-2 border-b border-slate-200/80 dark:border-slate-700/80">
                                ...
                                <div className="flex items-center justify-between min-w-0">
                                    ...
                                    {key.toLowerCase() === 'targa' && value && (
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleCopyTarga(); }} 
                                            title="Copia targa"
                                            className="p-1.5 text-slate-500 dark:text-gray-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md ml-2 flex-shrink-0"
                        ...
                        {isFacchetti ? (
                            <fieldset className="mb-4">
                                <legend className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-2">
                                    Scegli il modello di messaggio
                                </legend>
