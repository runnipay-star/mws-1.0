...
const LiveOverview: React.FC<{
    ...
    const overviewData = useMemo(() => {
        ...
        leads.forEach(lead => {
            const groupId = groupBy === 'client' ? lead.client_id! : (lead.service || 'Non specificato');
            
            if (!dataMap.has(groupId)) {
                let groupName = 'Sconosciuto';
                if (groupBy === 'client') {
                    groupName = clients.find(c => c.id === groupId)?.name || 'Sconosciuto';
                } else {
                    groupName = groupId;
                }
                dataMap.set(groupId, {
    ...
    if (leads.length === 0) {
         return (
             <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700">
                <p className="text-gray-500 dark:text-gray-400 text-center py-8">Nessun lead trovato per i filtri selezionati.</p>
            </div>
        )
    }
                    ...
                    {overviewData.map(item => (
                        <div key={item.groupName} className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700 space-y-2">
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
                                Totale Lead: <span className="font-semibold text-slate-700 dark:text-gray-300">{item.total}</span>
                            </p>
                                        ...
                                        <div key={status} className="border-b border-slate-200 dark:border-slate-700 last:border-b-0">
                                            ...
                                            >
                                                <div className="flex items-center">
                                                    <span className={`px-2 py-0.5 text-xs font-bold rounded-full ${statusColors[status]}`}>{status}</span>
                                                </div>
                                                <div className="flex items-center">
                                                ...
                                                <div id={`leads-for-${item.groupName}-${status}`} className="pb-2 pl-2 space-y-2 max-h-60 overflow-y-auto">
                                                    {leadsInStatus.map(lead => (
                                                        <div key={lead.id} className="text-xs p-2 bg-slate-100 dark:bg-slate-800 rounded-md">
                                                            <p className="font-medium text-slate-800 dark:text-gray-200">{lead.data.nome || 'Lead senza nome'}</p>
                                                            <p className="text-slate-500 dark:text-gray-400">{new Date(lead.created_at).toLocaleDateString('it-IT')}</p>
                                                            {groupBy === 'client' && <p className="text-slate-500 dark:text-gray-400">{lead.service || 'Nessun servizio'}</p>}
