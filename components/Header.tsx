...
import { LogOut, User as UserIcon, ChevronDown, Settings, FileText, MessageSquare, Sun, Moon, Bell, Send, Languages, Users, MapPin, Loader2 } from 'lucide-react';
        ...
        if (isOpen && users.length > 0) {
            const fetchLocations = async () => {
                ...
                const locationPromises = users.map(user => 
                    fetch(`https://ipapi.co/${user.ip}/json/`).then(res => res.json()).then(data => ({ id: user.id, data }))
                );
                try {
                    ...
                    {users.map(user => {
                        ...
                        if (locationData) {
                            if (locationData.error) {
                                ...
                                locationString = [locationData.city, locationData.region, locationData.country_name].filter(Boolean).join(', ');
                            }
                        }
                        return (
